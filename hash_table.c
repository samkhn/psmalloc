...
void hash_table_init(struct HashTable *ht, int capacity) {
    ...
    ht->addresses = (struct AddressSizePair *) mmap(NULL, capacity * sizeof(struct AddressSizePair), PROT_READ | PROT_WRITE, MAP_PRIVATE | MAP_ANONYMOUS, -1, 0);
...
void hash_table_insert(struct HashTable *ht, struct AddressSizePair address) {
    uint64_t hash = ((uint64_t) address.key * 11400954310080068983) % ht->capacity;
...
struct AddressSizePair *hash_table_search(struct HashTable *ht, void *address) {
    uint64_t hash = ((uint64_t) address * 11400954310080068983) % ht->capacity;
    struct AddressSizePair *pair = ht->addresses[hash].key;
    ...
    if (pair == address) {
        ...
        do {
            newhash = (newhash + 1) % ht->capacity;
            pair = ht->addresses[newhash].key;
        }
        while((pair != address) || newhash != hash);
        return NULL;
    }
    return NULL;
}
